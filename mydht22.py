...
def main():
	...
	while True:
		if(not dhtDefined):
			...
			choiceDHT = input("Sollen die Daten vom DHT22 in eine neue Datei gespeichert werden(1) oder in eine bereits bestehende(2)? (1/2)")
			...
			if choiceDHT == "1":
				...
				file.write("Zeit in JHR-MNT-TAG STD:MIN:SEK\tTemperatur in °C\tLuftfeuchte in %\n")
				file.close()
			
		########################################################
		if(not adxDefined):
			print("Konfiguriere ADXL345:")
			choiceADX = input("Sollen die Daten vom ADXL345 in eine neue Datei gespeichert werden(1) oder in eine bereits bestehende(2)? (1/2)")
			...
			if choiceADX == "1":
				file = open(filePathADX, 'w')
				# write the header of the new file:
				file.write("Zeit in JHR-MNT-TAG STD:MIN:SEK\tBeschleunigungX\tBeschleunigungY\tBeschleunigungZ\n")
				file.close()
			
		# go now into the main-program:
		print("Die Beschleunigungsdaten werden nun im Sekundentakt und die Temperaturwerte im Minutentakt in die angegebenen Dateien gespeichert.")
